<?php
    ...
    if (isset($_POST['signup'])) {
        ...
        // count erros
        if (count($errors) === 0) {
            $insertQuery = "INSERT INTO userInfo (firstName,lastName,email,password,gender,code,status)
            ...
            if ($insertInfo) {
                ...
                $sender = 'From: bankarprathamesh45@gmail.com'; 
    ...
    if (isset($_POST['verify'])) {
        ...
        $otp_query = "SELECT * FROM userInfo WHERE code = $otp";
        ...
        if (mysqli_num_rows($otp_result) > 0) {
            ...
            $update_query = "UPDATE userInfo SET status = '$update_status' , code = $update_code WHERE code = $fetch_code";
    ...
    if (isset($_POST['login'])) {
        ...
        if (mysqli_num_rows($email_check) > 0) {
            $passwordQuery = "SELECT * FROM userInfo WHERE email = '$email' AND password = '$password'";
    ...
    if (isset($_POST['forgot_password'])) {
        ...
        $emailCheckQuery = "SELECT * FROM userInfo WHERE email = '$email'";
...
if(isset($_POST['verifyEmail'])){
    ...
    $verifyQuery = "SELECT * FROM userInfo WHERE code = $OTPverify";
    $runVerifyQuery = mysqli_query($conn, $verifyQuery);
    if($runVerifyQuery){
        if(mysqli_num_rows($runVerifyQuery) > 0){
            $newQuery = "UPDATE userInfo SET code = 0";
...
if(isset($_POST['changePassword'])){
    ...
    if (strlen($_POST['password']) < 8) {
        ...
        if ($_POST['password'] != $_POST['confirmPassword']) {
            ...
            $updatePassword = "UPDATE userInfo SET password = '$password' WHERE email = '$email'";
